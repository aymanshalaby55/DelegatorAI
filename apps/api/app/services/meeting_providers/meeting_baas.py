import httpx
from fastapi import HTTPException

from app.core.config import get_settings
from app.services.meeting_providers.base import MeetingProvider, BotInfo, TranscriptData

settings = get_settings()


def _build_join_payload(meeting_url: str, bot_name: str) -> dict:
    return {
        "meeting_url": meeting_url,
        "bot_name": bot_name,
        "recording_mode": "speaker_view",
        "transcription_enabled": True,
        "transcription_config": {
            "provider": "gladia",
            "custom_params": {
                "summarization": True,
                "summarization_config": {"type": "bullet_points"},
                "language_config": {
                    "languages": ["en"],
                    "code_switching": True,
                },
                "translation": True,
                "translation_config": {"target_languages": ["en"]},
            },
        },
        "automatic_leave": {"waiting_room_timeout": 600},
        "webhook_url": settings.meeting_baas_webhook_url,
    }


def _raise_for_http_errors(exc: Exception) -> None:
    if isinstance(exc, httpx.HTTPStatusError):
        raise HTTPException(
            status_code=exc.response.status_code,
            detail={
                "message": "Meeting BaaS returned an error",
                "upstream_status": exc.response.status_code,
                "upstream_body": exc.response.text,
            },
        ) from exc
    if isinstance(exc, httpx.TimeoutException):
        raise HTTPException(
            status_code=504, detail="Meeting BaaS request timed out"
        ) from exc
    if isinstance(exc, httpx.RequestError):
        raise HTTPException(
            status_code=502,
            detail=f"Could not reach Meeting BaaS: {type(exc).__name__}",
        ) from exc
    raise exc


def _validate_config(api_key: str, base_url: str) -> None:
    missing = []
    if not api_key:
        missing.append("API key")
    if not base_url:
        missing.append("base URL")
    if not getattr(settings, "meeting_baas_webhook_url", None):
        missing.append("webhook URL")
    if missing:
        raise ValueError(f"Meeting BaaS not configured: {', '.join(missing)}")


class MeetingBaasProvider(MeetingProvider):
    def __init__(self) -> None:
        self.base_url = settings.meeting_baas_base_url
        self.api_key = settings.meeting_baas_api_key
        _validate_config(self.api_key, self.base_url)

    @property
    def _headers(self) -> dict:
        return {
            "x-meeting-baas-api-key": self.api_key,
            "Content-Type": "application/json",
        }

    @property
    def _headers_no_content_type(self) -> dict:
        return {k: v for k, v in self._headers.items() if k.lower() != "content-type"}

    async def join(self, meeting_url: str, bot_name: str) -> BotInfo:
        payload = _build_join_payload(meeting_url, bot_name)
        try:
            async with httpx.AsyncClient() as client:
                response = await client.post(
                    f"{self.base_url}/v2/bots",
                    json=payload,
                    headers=self._headers,
                    timeout=30,
                )
                response.raise_for_status()
                raw = response.json()
                return BotInfo(
                    bot_id=raw.get("data", {}).get("bot_id", "unknown"), raw=raw
                )
        except Exception as exc:
            _raise_for_http_errors(exc)

    async def leave(self, bot_id: str) -> None:
        if not bot_id:
            raise HTTPException(
                status_code=400, detail="Missing bot_id for leaving the meeting"
            )
        try:
            async with httpx.AsyncClient() as client:
                response = await client.post(
                    f"{self.base_url}/v2/bots/{bot_id}/leave",
                    headers=self._headers_no_content_type,
                    timeout=15,
                )
                if response.status_code == 409:
                    self._raise_if_already_completed(response)
                if response.status_code != 200:
                    raise HTTPException(
                        status_code=502,
                        detail=f"Meeting BaaS error {response.status_code}: {response.text}",
                    )
        except httpx.RequestError as exc:
            raise HTTPException(
                status_code=502, detail=f"Meeting BaaS connection error: {exc}"
            ) from exc

    async def get_transcript(self, bot_id: str) -> TranscriptData:
        if not bot_id:
            raise HTTPException(
                status_code=400, detail="Missing bot_id for getting transcript"
            )
        try:
            async with httpx.AsyncClient() as client:
                response = await client.get(
                    f"{self.base_url}/v2/bots/{bot_id}",
                    headers=self._headers,
                    timeout=20,
                )
                if not response.is_success:
                    raise HTTPException(
                        status_code=502,
                        detail=f"Meeting BaaS error {response.status_code}: {response.text}",
                    )
                data = response.json()
                return TranscriptData(
                    transcript=data.get("transcript", []),
                    recording_url=data.get("recording_url"),
                    speakers=data.get("speakers", []),
                )
        except httpx.RequestError as exc:
            raise HTTPException(
                status_code=502, detail=f"Meeting BaaS connection error: {exc}"
            ) from exc

    @staticmethod
    def _raise_if_already_completed(response: httpx.Response) -> None:
        try:
            error_json = response.json()
        except Exception:
            error_json = {}
        message = str(error_json.get("message", "")).lower()
        if (
            isinstance(error_json, dict)
            and error_json.get("code") == "FST_ERR_BOT_STATUS"
            and "completed" in message
            and "operation not permitted" in message
        ):
            raise HTTPException(
                status_code=409, detail="Meeting bot is already completed; cannot leave"
            )
