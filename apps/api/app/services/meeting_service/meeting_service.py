...
async def join_meeting(user: dict, data: JoinMeetingRequest, provider_name: str) -> dict:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to save meeting to database: {str(e)}")
...
async def leave_meeting(meeting_id: str, user_id: str, provider_name: str) -> dict:
    ...
    try:
        ...
        # Handle specific conflict error for bot status "completed"
        msg = str(e)
        if (
            ...
            return {
                "message": "Meeting already completed or left, marked as left"
            }
...
async def get_transcript(meeting_id: str, user_id: str, provider_name: str) -> dict:
    ...
    try:
        if not meeting.get("bot_id"):
            raise HTTPException(status_code=400, detail="Missing bot_id for transcript retrieval")
        transcript = await provider.get_transcript(meeting["bot_id"])
        if not transcript or not isinstance(transcript, dict):
            raise HTTPException(status_code=404, detail="Transcript not found or invalid response")
        return transcript
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=502, detail=f"Failed to get transcript: {str(e)}")